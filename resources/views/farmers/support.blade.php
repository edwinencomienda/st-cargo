...
<ul class="w3-ul w3-border list" id="myList" style="width:100%">
    ...
    <?php $cq =1 ;?>
    <?php
     $read = DB::table('reply')->where('farmer', Auth::user()->uuid)->where('reading', 'Unread')->latest()->get();
     foreach ($read as $con) {
    ?>
...
<h3 class="name">
   ...
   @php
   $const = DB::table('users')->where('uuid', $con->admin)->get();
...
<span class="text-center">
    Subject: &nbsp;&nbsp;&nbsp; {{$con->anstitle}}
</span>
<span class="w3-right">
    Date:  {{$con->created_at}}

    <a href="{{ URL::to('/farm/conversation/'.$con->id) }}" class="w3-hover-opacity"
...
&nbsp;
      <a  href="{{ URL::to('/delete/reply/'.$con->id) }}"
        onclick="return confirm('Do you really want to delete this message?')"
