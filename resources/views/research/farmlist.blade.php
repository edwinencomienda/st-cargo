    ...
    <table class="table table-bordered w3-card-4 display" id="inventory">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
              @php
            $bgt = DB::table('farmerprofile')->where('uuid', $inv->uuid)->get();
            foreach ($bgt as $bg) {
          ...
          <td>
              @php
                   $bgt = DB::table('baranggay')->where('id', $inv->location)->get();
            foreach ($bgt as $bg) {
           echo $bg->brgy;
            }
              @endphp
...
@else
<a href="{{ URL::to('/seelocationoffarm/'.$inv->id) }}" type="button" class="btn w3-brown"
    title="View the User Location">
...
@endif
            ...
            <a href="{{ URL::to('/delparcel/'.$inv->id) }}"
...
<div class="modal fade" id="addloc{{$inv->id}}" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    @php
    $a = $inv->id;
       $users = DB::table('farmparcel')->where('id', $a)->get();
       foreach ($users as $us) {
       }
   ...
   @endphp
                       ...
                       <div class="modal-body w3-display-container">
                           ...
                           <div class="w3-margin w3-padding-16">
                               <form action="{{url('/addparcellocation/add/'.$us->id)}}" method="POST">
                                   @csrf
                                   ...
                                   <p>
                                       ...
                                       <input class="w3-input" type="text" name="latt" required>
                                   </p>

                                   <p>
                                       <label class="w3-text-teal">Longgitude</label>
                                       <input class="w3-input" type="text" name="longi" required>
                                   </p>
