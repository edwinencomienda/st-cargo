...
<br><br>
            <div class="picture w3-col w3-white w3-padding-64" style="width:350px; display:none; margin-left:5px; margin-right:5px; padding-left:15px; padding-right:15px;" id="f{{$parc->id}}">
                ...
                <h3>Owner: <span class="w3-right" style="font-size: 15px;"> @php
                    $bgt = DB::table('farmerprofile')->where('uuid', $parc->farmer)->get();
                    foreach ($bgt as $bg) {
                ...
                <h3>
                    ...
                    $bgt = DB::table('baranggay')->where('id', $parc->parcel)->get();
                    foreach ($bgt as $bg) {
                    echo $bg->brgy;
                    }
                    @endphp
...
?>
                        ...
                        var <?php echo 'f' . $parc->id; ?> = new Microsoft.Maps.Pushpin(new Microsoft.Maps.Location('{{$parc->latt}}', '{{$parc->longi}}'),{color: '<?php switch ($parc->program) { case 2: echo "red";break;case 3: echo "orange"; break; case 4: echo "brown"; break; default: echo "black"; break; } ?>'});
                        <?php echo 'f' . $parc->id; ?>.metadata = {
                          title: 'Parcel of Farmer',
                          description: '@php $bgt = DB::table("farmerprofile")->where("uuid",$parc->farmer)->get();foreach ($bgt as $bg) {echo $bg->lname . " ". $bg->fname . ", " . $bg->mname; }@endphp <br><button class="w3-button btn-block w3-brown" onclick="openImg(\'f<?php echo $parc->id; ?>\');">Click</button>'
