...
function GetMap() {
    ...
    pin.metadata = {
        title: '@php $bgt = DB::table("farmerprofile")->where("uuid", $product->uuid)->get();foreach ($bgt as $bg) {echo $bg->lname . " ". $bg->fname . ", " . $bg->mname; }@endphp',
               ...
               <div class="w3-center w3-padding-32">
                ...
                <div class="w3-padding-64" style="padding-left: 15px; padding-right:15px;">
                    <h3>Owner: <span class="w3-right" style="font-size: 15px;"> @php
                        $bgt = DB::table('farmerprofile')->where('uuid', $product->uuid)->get();
                        foreach ($bgt as $bg) {
                    ...
                    <h3>
                        ...
                        $bgt = DB::table('baranggay')->where('id', $product->location)->get();
                        foreach ($bgt as $bg) {
                        echo $bg->brgy;
                        }
                        @endphp
                    ...
                    <h3>
                        Hectare: <span class="w3-right" style="font-size: 15px;">{{$product->hectare}} </span>
                    </h3>
                    ...
                    <div>
                        ...
                        <ul class="w3-ul w3-card-4" style="width:50%">
                            <?php
                            $bst = DB::table('parcelanimal')->where('uuid', $product->uuid)->where('parcel', $product->id)->get();
                           foreach ($bst as $bs) { ?>
                            <li>{{$bs->animal}}</li>
                            <?php   }
                            ?>
                    ...
                    <a href="{{route('map.list')}}" class="w3-button btn-block w3-brown">Go back to Parcel's List</a>
                </div>
              </div>
