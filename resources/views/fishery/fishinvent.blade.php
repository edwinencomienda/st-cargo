         ...
         <div class="modal-content w3-padding-32">
           ...
           <div class="modal-body w3-display-container">
               ...
               <div class="w3-margin w3-padding-16">
                   <form action="{{ route('fishinvent.store')}}" method="POST"  onsubmit="checkForm(this);">
                       @csrf

                       <div class="w3-row">
                        <div class="w3-container w3-half">
                            <p>
                                <label class="w3-text-teal">Select Fishery Program</label>
                        ...
                        <div class="w3-container w3-half">
                            <p id="one" style="display: none;">
                                <label class="w3-text-teal">Marine Program</label>
                                <select class="w3-select" name="products">
                                <option value="" disabled selected>Marine Program</option>
                                    <?php
                               ...
                               <p id="two"  style="display: none;">
                                <label class="w3-text-teal">Inland Aquatic Program</label>
                                <select class="w3-select" name="products" id="stwo">
                                <option value="" disabled selected>Inland Program</option>
                                    <?php
    ...
    <table class="table table-bordered w3-card-4 display" id="inventory">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{$inv->program}} -
          @php
              $sss = DB::table('fishservice')->where('id', $inv->program)->get();
          ...
          <td>
            @php
            $ttt = DB::table('fishproduct')->where('id', $inv->product)->get();
        ...
        @endphp
          ...
          <td class="text-right">
            ...
            <a href="{{ URL::to('/deletefishinventory/'.$inv->id) }}"
                onclick="return confirm('Do you really want to delete the inventory in fishery?')" type="button" class="btn btn-outline-danger"
 title="Delete">
    ...
    <div class="modal-dialog modal-dialog-centered">
         <div class="modal-content w3-padding-32">
           ...
           <div class="modal-body w3-display-container">
               ...
               @php
               $a = $inv->id;
               $tr = DB::table('fishinventory')->where('id', $a)->get();
               foreach ($tr as $tr) {
               }
                @endphp
               ...
               <div class="w3-margin w3-padding-16">
                <form action="{{ url('/fishinventory/edit/'.$tr->id)}}" method="POST">
                    @csrf
                       ...
                       <div class="w3-row">
                        <div class="w3-container w3-half">
                            <p>
                                <label class="w3-text-teal">Select Fishery Program</label>
                                <select class="w3-select" name="crop_program" required id="program">
                                <option value="{{$tr->program}}" selected>
                                @php
                                    $users = DB::table('fishservice')->where('id', $tr->program)->get();
                                    foreach ($users as $users){
                                        echo $users->description;
                                    }
                                @endphp
                                </option>
                                    <?php
                        ...
                        <div class="w3-container w3-half">
                               ...
                               <p id="two"  style="display: block;">
                                <label class="w3-text-teal">Fishery Programs</label>
                                <select class="w3-select" name="products" id="stwo">
                                <option value="{{$tr->product}}" selected>
                                @php
                                    $users = DB::table('fishproduct')->where('id', $tr->product)->get();
                                    foreach ($users as $users){
                                        echo $users->products;
                                    }
                                @endphp
                                    ...
                                    $users = DB::table('fishproduct')->get();
                                       foreach ($users as $users) { ?>
                                       <option value="{{$users->id}}" id="lk">{{$users->products}}</option>
                                <?php } ?>
                                  </select>
