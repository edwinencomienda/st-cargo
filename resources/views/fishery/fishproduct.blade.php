     ...
     <div class="modal-content w3-padding-32">
       ...
       <div class="modal-body w3-display-container">
           ...
           <div class="w3-margin w3-padding-16">
               <form action="{{ route('fishpro.store')}}" method="POST"  onsubmit="checkForm(this);">
                   @csrf
    ...
    <table class="table table-bordered w3-card-4 display" id="inventory">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>{{$inv->fish_program}}-
          @php
              $sss = DB::table('fishservice')->where('id', $inv->fish_program)->get();
          ...
          <td class="text-right">
            ...
            <a href="{{ URL::to('/deleteproduct/fishery/'.$inv->id) }}"
                onclick="return confirm('Do you really want to delete this fish products?')" type="button" class="btn btn-outline-danger"
 title="Delete">
                    ...
                    <div class="modal-dialog modal-dialog-centered">
                     <div class="modal-content w3-padding-32">
                       ...
                       <div class="modal-body w3-display-container">
                           ...
                           @php
                           $a = $inv->id;
                              $users = DB::table('fishproduct')->where('id', $a)->get();
                              foreach ($users as $u) {
                          ...
                          @endphp
                           <div class="w3-margin w3-padding-16">
                               <form action="{{ url('/fishproducta/'.$u->id)}}" method="POST"  onsubmit="checkFormsa(this);">
                                   @csrf

                                   <p>
                                    <label class="w3-text-teal w3-left">Fishery Program</label>
                                    <select class="w3-select" name="crop_program" required id="tract">
                                        <option value="{{$u->fish_program}}" selected>
                                            <?php
                                               $users = DB::table('fishservice')->where('id', $u->fish_program)->get();
