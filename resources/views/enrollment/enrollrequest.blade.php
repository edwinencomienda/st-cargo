    ...
    <div class="modal-dialog modal-xl modal-dialog-centered modal-dialog-scrollable">
      <div class="modal-content">
        ...
        <div class="modal-body">
            @php
            $a = $treq->id;
               $trs = DB::table('enrollment')->where('id', $a)->get();
               foreach ($trs as $tr) {
               }
            @endphp
                        ...
                        <li class="w3-display-container">
                            <h3>Farmer's Name</h3>
                             <span class="w3-button w3-transparent w3-display-right">
                                ...
                                ->select('lname', 'fname', 'mname')->where('uuid', $tr->farmer);
                                $users = DB::table('employeeprofile')
                                ->select('lname', 'fname', 'mname')
                                ->union($first)
                                ->where('uuid', $tr->farmer)
                                ->get();
                                foreach ($users as $u) { ?>
                        ...
                        <li class="w3-display-container">
                            <h3>Programs </h3>
                             <span class="w3-button w3-transparent w3-display-right">
                                @switch($treq->program)
                                @case(2)
                                <h3>Crop Enrollment</h3>

                                    @break
                                @case(3)
                                <h3>Fishery Enrollment</h3>
                                    @break

                                @case(4)
                               <h3>Livestock Enrollment</h3>
                                    @break
                                @default
                                None Register
                            @endswitch
...
<div class="w3-card-4 w3-card-4 w3-white w3-padding-64" style="padding-left:15px !important;padding-right:15px !important;">
    ...
    <form action="{{url('/enroll/updating/'.$tr->id)}}" method="POST"><br>
        @csrf

        <input class="w3-input" type="hidden" name="admin" value="{{Auth::user()->uuid}}" required>
      ...
      <div class="modal-content w3-padding-16 w3-card-4">
        ...
        <div class="modal-body">
           ...
           @endphp
            ...
            <div class="w3-bar w3-card-4">
                <a href="{{ URL::to('/deleteenroll/this/'.$tr->id) }}" type="button"
